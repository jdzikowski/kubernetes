...
// 3. All templates in the hierarchy share the exact same priority and PriorityClassName.
func ValidateWorkloadSpec(spec *scheduling.WorkloadSpec, fldPath *field.Path) field.ErrorList {
	...
	if len(spec.PodGroupTemplates) == 0 && len(spec.CompositePodGroupTemplates) == 0 {
		allErrs = append(allErrs, field.Required(fldPath, "exactly one of compositePodGroupTemplates and podGroupTemplates must be set"))
	}
	if len(spec.PodGroupTemplates) > 0 && len(spec.CompositePodGroupTemplates) > 0 {
		allErrs = append(allErrs, field.Forbidden(fldPath.Child("compositePodGroupTemplates"), "exactly one of compositePodGroupTemplates and podGroupTemplates must be set"))
	}

	configs := map[priorityConfig]bool{}
	for _, pg := range spec.PodGroupTemplates {
		configs[newPriorityConfig(pg.PriorityClassName, pg.Priority)] = true
	}
	for _, cpg := range spec.CompositePodGroupTemplates {
		configs[newPriorityConfig(cpg.PriorityClassName, cpg.Priority)] = true
		collectPriorityConfigs(configs, &cpg)
	}
	if len(configs) > 1 {
		allErrs = append(allErrs, field.Invalid(fldPath, nil, "detected multiple priority configurations"))
	}
...
func newPriorityConfig(className string, priority *int32) priorityConfig {
	var val int32
	if priority != nil {
		val = *priority
	}
	return priorityConfig{
		...
		hasPriority:       priority != nil,
		priority:          val,
	}
}

func collectPriorityConfigs(configs map[priorityConfig]bool, cpg *scheduling.CompositePodGroupTemplate) {
	for _, child := range cpg.CompositePodGroupTemplates {
		configs[newPriorityConfig(child.PriorityClassName, child.Priority)] = true
		collectPriorityConfigs(configs, &child)
	}
