...
func TestCache_GetRootGroup(t *testing.T) {
	setupCacheForRootGroupTest := func(ctx context.Context, genericWorkloadEnabled, compositePodGroupEnabled bool) *cacheImpl {
		c := newCache(ctx, time.Second, nil, genericWorkloadEnabled, compositePodGroupEnabled)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, ctx := ktesting.NewTestContext(t)
			c := setupCacheForRootGroupTest(ctx, tt.genericWorkloadEnabled, tt.compositePodGroupEnabled)
